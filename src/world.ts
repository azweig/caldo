...
export interface Creature {
  ...
  items?: string[]    // things they carry (gathered or received) — you can gift, use or collect them
}
...
export class World {
  ...
  static fromState(s: any, spriteCount: number): World {
    ...
    w.creatures = (s.creatures || []).map((c: any) => {
      ...
      cr.away = num(c.aw, 0); cr.awayTo = san(c.at, 40); cr.langs = sanArr(c.lg, 6, 40); cr.items = sanArr(c.it, 12, 24)
